...
Its button confirms the feed is copied, then provides Google’s official **From
URL** screen and the exact paste step. The old `cid` one-click shortcut is
unreliable and is intentionally not used. The dialog includes a copyable URL
and manual instructions for both platforms.
